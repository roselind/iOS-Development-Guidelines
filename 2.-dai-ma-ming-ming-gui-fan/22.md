...
* 方法名不要使用`new`作为前缀。
* 小写第一个单词的首字符,大写随后单词的首字符,不使用前缀。请参考[书写约定](21.md#书写约定)一节。有两种例外情况:1.方法名以广为人知的大写字母缩略词\(如 TIFF or PDF\)开头;2.私有方法可以使用统一的前缀来分组和辨识,请参考[私有方法](22.md#私有方法)一节
* 表示对象行为的方法,名称以动词开头:
...
* 如果方法返回方法接收者的某个属性,直接用属性名称命名。不要使用 get，除非是间接返回一个或多个值。请参考[访问方法](22.md#访问方法)一节。
